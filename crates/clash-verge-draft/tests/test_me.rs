...
mod tests {
    ...
    fn verge(enable_auto_launch: bool, enable_tun_mode: bool) -> IMeta {
        IMeta {
    ...
    fn test_edit_draft_returns_closure_result() {
        let draft = Draft::new(IMeta::default());
    ...
    fn test_with_data_modify_error_releases_permit() {
        ...
        let err = block_on_ready(draft.with_data_modify(|_v| async move { Err::<(IMeta, ()), _>(anyhow!("boom")) }))
            .unwrap_err();
