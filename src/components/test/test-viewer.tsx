...
interface Props {
  onChange: (uid: string, patch?: Partial<IMetaTestItem>) => void
}

export interface TestViewerRef {
  create: () => void
  edit: (item: IMetaTestItem) => void
}
...
export const TestViewer = forwardRef<TestViewerRef, Props>(
  ({ onChange }, ref) => {
    ...
    const { control, ...formIns } = useForm<IMetaTestItem>({
      defaultValues: {
        name: '',
        icon: '',
        url: '',
