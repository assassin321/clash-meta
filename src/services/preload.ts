...
let vergeConfigCache: IMetaConfig | null | undefined
...
const getThemeModeFromWindow = (): IMetaConfig['theme_mode'] | undefined => {
...
export const setPreloadConfig = (config: IMetaConfig | null) => {
...
const preloadConfig = async () => {
  try {
    ...
    console.warn('[preload.ts] Failed to read Meta config:', error)
...
const preloadLanguage = async (
  vergeConfig?: IMetaConfig | null,
  loadConfig: () => Promise<IMetaConfig | null> = preloadConfig,
) => {
  ...
  if (resolvedConfig === undefined) {
    try {
      ...
      console.warn(
        '[preload.ts] Failed to read language from Meta config:',
        error,
      )
      resolvedConfig = null
    }
