...
const buildCommand =
  'TAURI_CONFIG=<scripts/perf/tauri.json> cargo build -p clash-meta --release --features perf-harness --locked --target-dir target/perf/cargo'
execFileSync(
  'cargo',
  [
    'build',
...
const binary = `${output}/clash-meta`
copyFileSync('target/perf/cargo/release/clash-meta', binary)
