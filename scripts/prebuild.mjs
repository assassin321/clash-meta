...
function clashMetaAlpha() {
  ...
  return {
    name: 'meta-mihomo-alpha',
    targetFile: `meta-mihomo-alpha-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
...
function clashMeta() {
  ...
  return {
    name: 'meta-mihomo',
    targetFile: `meta-mihomo-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
    exeFile: `${name}${isWin ? '.exe' : ''}`,
...
const resolveServicePermission = async () => {
  const serviceExecutables = [
    'clash-meta-service*',
    'clash-meta-service-install*',
    'clash-meta-service-uninstall*',
  ]
  const hashCache = await loadHashCache()
