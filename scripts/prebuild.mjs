...
function clashMetaAlpha() {
  ...
  return {
    name: 'clash-mihomo-alpha',
    targetFile: `clash-mihomo-alpha-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
...
function clashMeta() {
  ...
  return {
    name: 'clash-mihomo',
    targetFile: `clash-mihomo-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
    exeFile: `${name}${isWin ? '.exe' : ''}`,
...
function serviceFileInfo(name) {
  ...
  return {
    sourceFile: `${name}${ext}`,
    targetFile: `${name.replace('clash-verge-', 'clash-meta-')}${suffix}${ext}`,
  }
}
...
async function resolveCoreHashes() {
  ...
  for (const [define, name] of [
    ['MIHOMO_SHA256', 'clash-mihomo'],
    ['MIHOMO_ALPHA_SHA256', 'clash-mihomo-alpha'],
  ]) {
