...
!macro CheckAllVergeProcesses
  ...
  ; Check if meta-mihomo-alpha.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "meta-mihomo-alpha.exe"
  !else
    nsis_tauri_utils::FindProcess "meta-mihomo-alpha.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill meta-mihomo-alpha.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "meta-mihomo-alpha.exe"
    !else
      nsis_tauri_utils::KillProcess "meta-mihomo-alpha.exe"
    !endif
  ${EndIf}

  ; Check if meta-mihomo.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "meta-mihomo.exe"
  !else
    nsis_tauri_utils::FindProcess "meta-mihomo.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill meta-mihomo.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "meta-mihomo.exe"
    !else
      nsis_tauri_utils::KillProcess "meta-mihomo.exe"
    !endif
  ${EndIf}
