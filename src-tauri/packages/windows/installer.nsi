...
!macro CheckAllVergeProcesses
  ; Check if clash-meta-service.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "clash-meta-service.exe"
  !else
    nsis_tauri_utils::FindProcess "clash-meta-service.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill clash-meta-service.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "clash-meta-service.exe"
    !else
      nsis_tauri_utils::KillProcess "clash-meta-service.exe"
    !endif
  ${EndIf}
...
Section Install
  ...
  Delete "$APPDATA\io.github.assassin321.clash-meta\window-state.json"
  Delete "$APPDATA\io.github.assassin321.clash-meta\.window-state.json"
  ...
  SetRegView 64
  ReadRegStr $R2 HKCU "$R1" "Clash Meta"
  ${If} $R2 != ""
    DeleteRegValue HKCU "$R1" "Clash Meta"
  ${EndIf}
  ReadRegStr $R2 HKLM "$R1" "Clash Meta"
  ${If} $R2 != ""
    DeleteRegValue HKLM "$R1" "Clash Meta"
  ${EndIf}
  ReadRegStr $R2 HKCU "$R1" "clash-meta"
  ${If} $R2 != ""
    DeleteRegValue HKCU "$R1" "clash-meta"
  ...
  IfFileExists "$INSTDIR\Clash Meta.exe" 0 +2
    Delete "$INSTDIR\Clash Meta.exe"
  ...
  !ifdef MIHOMO_SHA256
    DetailPrint "Staging clash-mihomo for ${PRODUCTNAME} Service..."
    nsExec::ExecToLog '"$INSTDIR\resources\clash-meta-service-install.exe" --install-core "$INSTDIR\clash-mihomo.exe" --sha256 "${MIHOMO_SHA256}"'
    Pop $0
    ${IfThen} $0 != 0 ${|} DetailPrint "Staging clash-mihomo returned $0" ${|}
  !endif
  !ifdef MIHOMO_ALPHA_SHA256
    DetailPrint "Staging clash-mihomo-alpha for ${PRODUCTNAME} Service..."
    nsExec::ExecToLog '"$INSTDIR\resources\clash-meta-service-install.exe" --install-core "$INSTDIR\clash-mihomo-alpha.exe" --sha256 "${MIHOMO_ALPHA_SHA256}"'
    Pop $0
    ${IfThen} $0 != 0 ${|} DetailPrint "Staging clash-mihomo-alpha returned $0" ${|}
  !endif
...
Section Uninstall
  ...
  Delete "$INSTDIR\clash-mihomo.old"
  Delete "$INSTDIR\clash-mihomo-alpha.old"
  ...
  ${If} $UpdateMode <> 1
    ...
    Delete "C:\Users\Public\Desktop\Clash Meta.lnk"
    Delete "C:\Users\Public\Desktop\clash-meta.lnk"
    ...
    SetShellVarContext current
    RMDir /r /REBOOTOK "$SMPROGRAMS\Clash Meta"
    RMDir /r /REBOOTOK "$SMPROGRAMS\clash-meta"
    !insertmacro SetContext
    RMDir /r /REBOOTOK "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Clash Meta"
    RMDir /r /REBOOTOK "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\clash-meta"

    ; Clean legacy registry keys
    SetRegView 64
    DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Clash Meta.exe"
    DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\clash-meta.exe"
    DeleteRegKey HKLM "Software\Clash Meta"
    DeleteRegKey HKLM "Software\Clash Meta"
    DeleteRegKey HKCU "Software\Clash Meta"
    DeleteRegKey HKCU "Software\Clash Meta"
    DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\ClashMeta"
    DeleteRegKey HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Clash Meta"
    ...
    LegacyUninstallLoop:
      ...
      ${If} $R3 != ""
        StrCmp $R3 "Clash Meta" 0 +3
        StrCmp $R3 "clash-meta" 0 +2
