...
fn service_bypass(verge: &IMeta) -> Result<String> {
...
fn service_proxy_config(verge: &IMeta, mixed_port: u16, pac_port: u16) -> Result<MacosProxyConfig> {
...
async fn current_service_proxy_config(verge: &IMeta) -> Result<MacosProxyConfig> {
    if !verge.enable_system_proxy.unwrap_or_default() {
        return Ok(MacosProxyConfig::Disabled);
    }
    if verge.proxy_auto_config.unwrap_or_default() {
...
mod tests {
    ...
    use crate::{config::IMeta, core::manager::RunningMode};
    use clash_verge_service_ipc::{MacosProxyConfig, OwnerSessionProof, ProxyApplyOutcome};
