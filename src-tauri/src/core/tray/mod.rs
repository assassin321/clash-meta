use crate::config::{IProfilePreview, IMeta};
...
pub const TRAY_ID: &str = "clash-meta-tray";
...
impl TrayState {
    async fn get_tray_icon(verge: &IMeta) -> (bool, Cow<'_, [u8]>) {
    ...
    async fn load_icon(verge: &IMeta, kind: IconKind) -> (bool, Cow<'_, [u8]>) {
    ...
    fn default_icon(verge: &IMeta, kind: IconKind) -> (bool, Cow<'_, [u8]>) {
        #[cfg(target_os = "macos")]
        {
...
impl Tray {
    ...
    /// 更新托盘图标
    pub async fn update_icon(&self, verge: &IMeta) -> Result<()> {
        if handle::Handle::global().is_exiting() {
