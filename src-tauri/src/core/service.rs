...
const fn macos_cleanup_translocated_desired_state_shell() -> &'static str {
    "for f in '/var/root/.local/state/clash-meta-service/desired-state.json' '/var/lib/clash-meta-service/desired-state.json'; do if [ -f \"$f\" ] && /usr/bin/grep -q AppTranslocation \"$f\"; then backup=\"$f.apptranslocation.bak\"; if [ -e \"$backup\" ]; then backup=\"$f.apptranslocation.$(/bin/date +%s).bak\"; fi; /bin/mv \"$f\" \"$backup\"; fi; done"
}

/// 卸载服务前以 root 清理残留 core 和 IPC 套接字。
#[cfg(target_os = "macos")]
fn macos_force_stop_core_shell() -> String {
    use crate::config::IMeta;

    // 只清理 root 拥有的服务内核。
    let mut parts: Vec<String> = IMeta::VALID_CLASH_CORES
        .iter()
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = binary_path.with_file_name("clash-meta-service-uninstall.exe");
...
fn install_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let install_path = binary_path.with_file_name("clash-meta-service-install.exe");
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = tauri::utils::platform::current_exe()?.with_file_name("clash-meta-service-uninstall");
...
fn install_service() -> Result<()> {
    ...
    let install_path = tauri::utils::platform::current_exe()?.with_file_name("clash-meta-service-install");
...
fn uninstall_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let uninstall_path = binary_path.with_file_name("clash-meta-service-uninstall");
...
fn install_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let install_path = binary_path.with_file_name("clash-meta-service-install");
...
mod tests {
    ...
    fn test_dir(name: &str) -> std::io::Result<PathBuf> {
        let path = std::env::temp_dir().join(format!("clash-meta-service-path-test-{}-{name}", std::process::id()));
    ...
    fn detects_app_translocation_paths() {
        let path = Path::new(
            "/private/var/folders/example/T/AppTranslocation/123/d/Clash Meta.app/Contents/MacOS/Clash Meta",
        );
    ...
    fn extracts_app_bundle_name_from_executable_path() {
        let path = Path::new("/Applications/Clash Meta.app/Contents/MacOS/Clash Meta");

        assert_eq!(
            macos_app_bundle_name(path).as_deref(),
            Some(std::ffi::OsStr::new("Clash Meta.app"))
        );
    }
    ...
    fn resolves_existing_core_path_from_install_roots() -> std::io::Result<()> {
        ...
        let core_dir = root.join("Clash Meta.app").join("Contents").join("MacOS");
        let core_path = core_dir.join("meta-mihomo");

        fs::create_dir_all(&core_dir)?;
        fs::write(&core_path, b"")?;

        let resolved = macos_core_path_in_install_roots(
            std::ffi::OsStr::new("Clash Meta.app"),
            "meta-mihomo",
            [root.as_path()],
        );
