    ...
    F: FnOnce(&Path, &Path) -> Result<()>,
{
    ...
    let staging_directory = home
        .join("Applications/.clash-meta-dev")
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = packaged_service_tool_path("clash-meta-service-uninstall.exe", || {
        Ok(dirs::service_path()?.with_file_name("clash-meta-service-uninstall.exe"))
    })?;
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = packaged_service_tool_path("clash-meta-service-uninstall", || {
        Ok(tauri::utils::platform::current_exe()?.with_file_name("clash-meta-service-uninstall"))
    })?;
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = packaged_service_tool_path("clash-meta-service-uninstall", || {
        Ok(dirs::service_path()?.with_file_name("clash-meta-service-uninstall"))
    })?;
...
fn install_service() -> Result<()> {
    ...
    let cores = crate::config::IMeta::VALID_CLASH_CORES
        .iter()
        .map(|core| {
...
fn invoke_service_install(cores: &[clash_verge_service_ipc::management::CoreSource], core_only: bool) -> Result<()> {
    let name = format!("clash-meta-service-install{}", std::env::consts::EXE_SUFFIX);
...
pub(super) async fn stage_runtime_by_service(config_file: &Path) -> Result<StageRequest> {
    ...
    let response = clash_verge_service_ipc::stage_runtime(&credentials, &session, &runtime)
        .await
        .context("无法连接到Clash Meta Service")?;
    if response.code > 0 {
    ...
    response
        ...
        .context("Clash Meta Service 未返回运行时暂存结果")
}
...
pub(super) async fn start_with_existing_service(config_file: &Path) -> Result<()> {
    ...
    let response = match clash_verge_service_ipc::start_clash(&credentials, &request).await {
        ...
        Err(error) => {
            ...
            start_owner_monitor();
            return Err(error).context("无法连接到Clash Meta Service");
        }
    };
    ...
    let result = response.data.context("Clash Meta Service 未返回会话信息")?;
...
pub(super) async fn get_clash_logs_by_service() -> Result<Vec<String>> {
    ...
    .await;
    let response = response.context("无法连接到Clash Meta Service")?;
...
pub(super) async fn set_system_proxy_by_service_with_session(
    ...
    response.data.context("Clash Meta Service 未返回系统代理结果")
}
...
mod tests {
    ...
    impl TestDirectory {
        fn new(label: &str) -> anyhow::Result<Self> {
            ...
            let path = std::env::temp_dir().join(format!(
                "clash-meta-service-{label}-{}-{generation}",
                std::process::id()
            ));
    ...
    fn staging_directory(home: &Path) -> PathBuf {
        home.join("Applications/.clash-meta-dev/service-core")
    }

    #[cfg(unix)]
    fn service_tools_staging_directory(home: &Path) -> PathBuf {
        home.join("Applications/.clash-meta-dev/service-tools")
    }
    ...
    fn nondevelopment_service_core_selection_preserves_sibling_without_staging() -> anyhow::Result<()> {
        ...
        let source = root.path().join("target/debug/clash-mihomo");
    ...
    fn development_service_core_uses_exact_layout_and_executable_bytes() -> anyhow::Result<()> {
        ...
        let home = root.path().join("home");
        let source = root.path().join("clash-mihomo");
        ...
        assert_eq!(
            selected,
            home.join("Applications/.clash-meta-dev/service-core/clash-mihomo")
        );
    ...
    fn development_service_tool_uses_safe_layout_and_executable_bytes() -> anyhow::Result<()> {
        ...
        let home = root.path().join("home");
        let source = root.path().join("clash-meta-service-install");
        ...
        assert_eq!(
            selected,
            service_tools_staging_directory(&home).join("clash-meta-service-install")
        );
    ...
    fn development_service_core_refresh_atomically_replaces_bytes() -> anyhow::Result<()> {
        ...
        assert!(staging_temporary_entries(&home, "clash-mihomo")?.is_empty());
        Ok(())
    }
    ...
    fn development_service_core_replaces_final_symlink_without_following_it() -> anyhow::Result<()> {
        ...
        let final_path = home.join("Applications/.clash-meta-dev/service-core/clash-mihomo");
        ...
        for code in [
            ...
            let error = super::record_service_start_refusal(
                ...
                super::ServiceStartRefusal {
                    code: code as u16,
                    core_path: "/development/service-core/clash-mihomo".into(),
    ...
    fn legacy_socket_alone_is_not_install_evidence() {
        assert!(
            !super::macos_service_install_markers()
                .iter()
                .any(|marker| marker == "/tmp/verge/clash-meta-service.sock")
        );
    }
