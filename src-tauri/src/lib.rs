...
pub fn run() -> std::process::ExitCode {
        ...
        utils::dirs::preinit_app_data_dir().and_then(|root| core::owner_identity::repair_app_data_root_owner(&root))
    {
        ...
        eprintln!("[clash-meta] 应用数据目录所有权修复失败: {error:#}");
    ...
    let builder = app_init::setup_plugins(tauri::Builder::default())
        .setup(|app| {
            ...
            fn log_setup_panic(stage: &str, panic: Box<dyn std::any::Any + Send>) {
                ...
                eprintln!("[clash-meta] panic during app setup ({stage}), continuing in degraded mode: {msg}");
                logging!(
                    error,
                    Type::Setup,
    ...
    mod event_handlers {
        ...
        pub fn handle_ready_resumed(_app_handle: &AppHandle) {
            ...
            if let Some(window) = _app_handle.get_webview_window("main") {
                let _ = window.set_title("Clash Meta");
            }
        }
