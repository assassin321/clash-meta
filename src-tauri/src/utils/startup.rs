...
pub(crate) fn report_error(error: &anyhow::Error) {
    ...
    eprintln!("[clash-meta] startup failed: {detail}");
    ...
    let message = match log_result {
        Ok(path) => format!(
            "Clash Meta could not start.\n\n{detail}\n\nDiagnostic log:\n{}",
            path.display()
        ),
        Err(log_error) => {
            eprintln!("[clash-meta] failed to write startup log: {log_error:#}");
            format!(
    ...
    let _ = rfd::MessageDialog::new()
        ...
        .set_title("Clash Meta startup failed")
