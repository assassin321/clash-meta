...
pub static APP_ID: &str = "io.github.assassin321.clash-meta";
#[cfg(not(feature = "verge-dev"))]
pub static BACKUP_DIR: &str = "clash-meta-backup";

#[cfg(feature = "verge-dev")]
pub static APP_ID: &str = "io.github.assassin321.clash-meta.dev";
#[cfg(feature = "verge-dev")]
...
pub fn service_path() -> Result<PathBuf> {
    ...
    Ok(res_dir.join("clash-meta-service"))
}

#[cfg(windows)]
pub fn service_path() -> Result<PathBuf> {
    let res_dir = app_resources_dir()?;
    Ok(res_dir.join("clash-meta-service.exe"))
}
...
fn sidecar_ipc_path_for(app_root: &std::path::Path, _identity: &clash_verge_service_ipc::OwnerIdentity) -> PathBuf {
    app_root.join("clash-mihomo.sock")
}
...
fn sidecar_ipc_path_for(
    ...
    #[cfg(feature = "verge-dev")]
    let filename = "clash-mihomo-dev.sock";
    #[cfg(not(feature = "verge-dev"))]
    let filename = "clash-mihomo.sock";
...
fn sidecar_pipe_name(identity: &clash_verge_service_ipc::OwnerIdentity, is_dev: bool) -> String {
    ...
    format!(
        r"\\.\pipe\clash-mihomo-sidecar-{flavor}-{}",
...
mod ipc_tests {
    ...
    fn sidecar_ipc_stays_in_the_app_root() {
        ...
        let app_root = Path::new("/home/test/.local/share/io.github.assassin321.clash-meta");
        let path = sidecar_ipc_path_for(app_root, &identity);

        assert_eq!(path, app_root.join("clash-mihomo.sock"));
        assert_ne!(
...
mod ipc_tests {
    ...
    fn sidecar_ipc_ignores_long_app_root_and_fits_sockaddr_un() -> anyhow::Result<()> {
        let identity = OwnerIdentity::Unix { uid: 501, gid: 20 };
        let app_root =
            Path::new("/Users/support/Library/Application Support/io.github.assassin321.clash-meta.dev");
        let path = sidecar_ipc_path_for(app_root, &identity)?;

        assert!(!path.starts_with(app_root));
        assert!(path.as_os_str().as_bytes().len() < 104);
        #[cfg(feature = "verge-dev")]
        assert_eq!(path.file_name(), Some(OsStr::new("clash-mihomo-dev.sock")));
        #[cfg(not(feature = "verge-dev"))]
        assert_eq!(path.file_name(), Some(OsStr::new("clash-mihomo.sock")));
...
mod windows_pipe_name_tests {
    ...
    fn windows_sidecar_pipe_separates_dev_and_release_for_the_same_owner() {
        ...
        assert_eq!(
            ...
            format!(r"\\.\pipe\clash-mihomo-sidecar-release-{owner_key}")
        );
        assert_eq!(
            sidecar_pipe_name(&identity, true),
            format!(r"\\.\pipe\clash-mihomo-sidecar-dev-{owner_key}")
        );
    }
}
