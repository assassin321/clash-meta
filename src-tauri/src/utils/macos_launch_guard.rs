...
mod tests {
    ...
    fn executable(bundle: &std::path::Path) -> anyhow::Result<std::path::PathBuf> {
        let executable = bundle.join("Contents/MacOS/clash-meta");
    ...
    fn canonical_allowed_roots_and_nested_bundles_are_accepted() -> anyhow::Result<()> {
        ...
        let system_exe = executable(&system.join("Tools/Clash Meta.app"))?;
        let user_exe = executable(&user.join("Network/Clash Meta.app"))?;
    ...
    fn failed_staged_activation_preserves_existing_application() -> anyhow::Result<()> {
        ...
        let destination = root.join("Clash Meta.app");
        let staging = root.join(".Clash Meta.app.installing");
        let backup = root.join(".Clash Meta.app.backup");
    ...
    fn canonicalization_accepts_symlink_in_and_rejects_symlink_escape() -> anyhow::Result<()> {
        ...
        let allowed_bundle = system.join("Clash Meta.app");
        let allowed_exe = executable(&allowed_bundle)?;
        let link_in = downloads.join("Clash Meta.app");
        ...
        assert!(matches!(
            evaluate_install_location_with_roots(&link_in.join("Contents/MacOS/clash-meta"), &home, &system),
            LaunchLocation::Allowed { .. }
        ));
        assert!(matches!(
            evaluate_install_location_with_roots(&link_out.join("Contents/MacOS/clash-meta"), &home, &system),
            LaunchLocation::Movable { .. }
        ));
    ...
    fn allowed_root_symlink_escape_is_not_authorized() -> anyhow::Result<()> {
        ...
        let escaped_exe = executable(&home.join("Applications/Clash Meta.app"))?;
    ...
    fn translocation_and_missing_bundle_are_rejected_before_side_effects() {
        let translocated =
            std::path::Path::new("/private/var/folders/AppTranslocation/Clash Meta.app/Contents/MacOS/clash-meta");
        assert_eq!(
        ...
        assert!(matches!(
            evaluate_install_location_with_roots(
                std::path::Path::new("/Users/test/Downloads/clash-meta"),
                std::path::Path::new("/Users/test"),
                std::path::Path::new("/Applications")
            ),
