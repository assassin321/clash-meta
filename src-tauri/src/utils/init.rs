...
use crate::{
    config::{Config, IClashTemp, IProfiles, IMeta},
    constants,
    core::handle,
    logging,
...
pub(super) async fn init_dns_config() -> Result<()> {
    ...
    if !dns_path.exists() {
        ...
        help::save_yaml(&dns_path, &default_dns_config, Some("# Clash Meta DNS Config")).await?;
    }
...
async fn initialize_config_files() -> Result<()> {
        ...
        && !path.exists()
    {
        ...
        help::save_yaml(&path, &template, Some("# Clash Meta"))
            .await
...
pub fn init_scheme() -> Result<()> {
    ...
    clash.set_value("", &"Clash Meta")?;
    clash.set_value("URL Protocol", &"Clash Meta URL Scheme Protocol")?;
...
#[cfg(target_os = "linux")]
pub fn init_scheme() -> Result<()> {
    const DESKTOP_FILE: &str = "clash-meta.desktop";
...
#[cfg(target_os = "linux")]
const DEEP_LINK_SCHEMES: &[&str] = &["clash", "clash-meta"];
