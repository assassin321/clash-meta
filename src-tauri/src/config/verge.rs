...
impl IMeta {
    pub const VALID_CLASH_CORES: &'static [&'static str] = &["clash-mihomo", "clash-mihomo-alpha"];
        ...
        if let Some(ref core) = config.clash_core {
            ...
            if core_str.is_empty() || !Self::VALID_CLASH_CORES.contains(&core_str) {
                logging!(
                    warn,
                    Type::Config,
                    "启动时发现无效的clash_core配置: '{}', 将自动修正为 'clash-mihomo'",
                    core
                );
                config.clash_core = Some("clash-mihomo".into());
                needs_fix = true;
            }
        } else {
            logging!(
                info,
                Type::Config,
                "启动时发现未配置clash_core, 将设置为默认值 'clash-mihomo'"
            );
            config.clash_core = Some("clash-mihomo".into());
            needs_fix = true;
        }
    ...
    pub fn get_valid_clash_core(&self) -> String {
        self.clash_core.clone().unwrap_or_else(|| "clash-mihomo".into())
    }
