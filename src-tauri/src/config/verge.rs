...
pub struct IMeta {
    ...
    /// theme setting
    pub theme_setting: Option<IMetaTheme>,
    ...
    /// 测试站列表
    pub test_list: Option<Vec<IMetaTestItem>>,
...
impl IMeta {
    /// 有效的clash核心名称
    pub const VALID_CLASH_CORES: &'static [&'static str] = &["meta-mihomo", "meta-mihomo-alpha"];
    ...
    pub async fn validate_and_fix_config() -> Result<()> {
        ...
        if let Some(ref core) = config.clash_core {
            ...
            if core_str.is_empty() || !Self::VALID_CLASH_CORES.contains(&core_str) {
                logging!(
                    warn,
                    Type::Config,
                    "启动时发现无效的clash_core配置: '{}', 将自动修正为 'meta-mihomo'",
                    core
                );
                config.clash_core = Some("meta-mihomo".into());
                needs_fix = true;
            }
        } else {
            logging!(
                info,
                Type::Config,
                "启动时发现未配置clash_core, 将设置为默认值 'meta-mihomo'"
            );
            config.clash_core = Some("meta-mihomo".into());
            needs_fix = true;
        }
        ...
        if needs_fix {
            ...
            help::save_yaml(&config_path, &config, Some("# Clash Meta Config")).await?;
    ...
    pub fn get_valid_clash_core(&self) -> String {
        self.clash_core.clone().unwrap_or_else(|| "meta-mihomo".into())
    }
    ...
    pub fn template() -> Self {
        Self {
            ...
            clash_core: Some("meta-mihomo".into()),
    ...
    /// Save IMeta App Config
    pub async fn save_file(&self) -> Result<()> {
        help::save_yaml(&dirs::verge_path()?, &self, Some("# Clash Meta Config")).await
    }
