use super::{Config, IClashTemp, IMeta, MixedPort};
use crate::{
    constants::timing,
    core::{
        handle::Handle,
...
impl Config {
    ...
    /// every launch. `config.yaml` and `meta.yaml` keep naming `old_port` so the next launch
    /// asks for it again.
...
fn configured_listener_ports(clash: &IClashTemp, verge: &IMeta) -> HashSet<u16> {
...
mod tests {
    ...
    use crate::config::{IClashTemp, IMeta};

    #[test]
    fn configured_ports_include_disabled_listener_assignments() {
        let ports = configured_listener_ports(&IClashTemp::template(), &IMeta::template());
